...
$testClaim=aslhub_authenticate($pdo,'test','test',aslhub_claim_password());
check($testClaim!==null && $testClaim['is_unclaimed']==1,'test account follows student claiming');
check(aslhub_save_claim_password($pdo,$testClaim['id'],'chosen-test-password'),'test account saves chosen password');
check(aslhub_authenticate($pdo,'test','test',aslhub_claim_password())===null,'test initial credential disabled after claiming');
check(aslhub_authenticate($pdo,'test','test','chosen-test-password')!==null,'test chosen password authenticates');
