...
- Target cards can set `promptTitle` and `promptText` to explain exactly why the player is picking a token. If omitted, a target card immediately followed by a damage effect on that target group gets a generic "Pick Enemy to Damage" style prompt. Token target cards with custom or inferred prompt text use one compact board picker prompt; optional picks show `Skip` there. The popup wording does not control damage; `damage.amount`, `damage.attribute`, `damage.amountDice`, and `damage.damageType` on the later effect still do.
