...
2. For class abilities, open the matching whole-class file in `source/rules/classes/`.
...
The books are split coarsely so an LLM does not need to load the full extracted text. Classes are one file per class. Monsters are grouped by nearby book sections instead of one file per monster or ability.
