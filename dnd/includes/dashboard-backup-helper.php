<?php
/**
 * Smart Backup System for Dashboard Character Data
 * Implements 3-tier backup strategy: Recent, Session, Daily
 * Based on Templates/GM Screen backup system architecture
 */

class DashboardBackupHelper {
    private $dataFile;
    private $backupDir;
    private $backupTypes = [
     ...
     * @param string $filePath Path to file to backup (optional, defaults to characters.json)
    ...
    private function createRecentBackup($content) {
        $backupFile = $this->backupDir . '/characters_recent_latest.json';
    ...
    private function createSessionBackup($content) {
        ...
        $backupFile = $this->backupDir . "/characters_session_{$slotNumber}_{$timestamp}.json";
    ...
    private function createDailyBackup($content) {
        ...
        if (!empty($todayBackups)) {
            ...
            $date = date('Y-m-d');
            $backupFile = $this->backupDir . "/characters_daily_{$date}.json";
        }
    ...
    private function getBackupsByType($type) {
        ...
        $pattern = $this->backupDir . "/characters_{$prefix}_*.json";
        $files = glob($pattern);
    ...
    private function getTodaysBackups() {
        $date = date('Y-m-d');
        $pattern = $this->backupDir . "/characters_daily_{$date}.json";
        $files = glob($pattern);
    ...
    public function getStats() {
        try {
            ...
            $backups = glob($this->backupDir . '/characters_*.json');
            if (!$backups) {
                return $stats;
            }
     ...
     */
    public function getBackups($fileName = 'characters.json') {
...
function createDashboardBackup($filePath, $type = 'auto') {
    $backupHelper = new DashboardBackupHelper();
