...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($_POST['action'] ?? '') {
        ...
        case 'restore_backup':
            ...
            $result = $backupHelper->restoreBackup($backupPath);
            echo json_encode($result);
            exit;
...
<head>
    ...
    <title>Data Recovery - Templates</title>
...
<body>
    <div class="recovery-container">
        <div class="recovery-header">
            <h1>Templates Data Recovery & Backup Management</h1>
        ...
        <div class="recovery-section">
            ...
            <?php if (empty($backups)): ?>
                <div class="empty-state">
                    ...
                    <p>No backups found. Backups are created automatically when templates are saved.</p>
                </div>
            <?php else: ?>
    ...
    <script>
        ...
        async function createManualBackup() {
            try {
                ...
                const response = await fetch('templates-recovery.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (result.success) {
                    showStatus('Templates backup created successfully!', 'success');
            ...
            try {
                ...
                if (result.success) {
                    if (result.valid_json) {
                        showStatus(`Backup is valid! Contains ${result.records} template records. Size: ${(result.size / 1024).toFixed(2)} KB`, 'success');
                    } else {
        ...
        async function restoreBackup(backupPath) {
            if (!confirm('Are you sure you want to restore from this backup? Current template data will be replaced.')) {
                return;
            }
            ...
            try {
                ...
                if (result.success) {
                    showStatus('Templates backup restored successfully!', 'success');
