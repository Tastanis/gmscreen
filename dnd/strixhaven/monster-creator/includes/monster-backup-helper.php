...
class MonsterBackupHelper {
    ...
    public function __construct($dataDir = null) {
        ...
        $this->dataFile = $dataDir . '/gm-monsters.json';
     ...
     * @param string $filePath Path to file to backup (optional, defaults to gm-monsters.json)
    ...
    private function createRecentBackup($content) {
        $backupFile = $this->backupDir . '/gm-monsters_recent_latest.json';
    ...
    private function createSessionBackup($content) {
        ...
        $backupFile = $this->backupDir . "/gm-monsters_session_{$slotNumber}_{$timestamp}.json";
    ...
    private function createDailyBackup($content) {
        ...
        if (!empty($todayBackups)) {
            ...
            $date = date('Y-m-d');
            $backupFile = $this->backupDir . "/gm-monsters_daily_{$date}.json";
        }
    ...
    private function getBackupsByType($type) {
        ...
        $pattern = $this->backupDir . "/gm-monsters_{$prefix}_*.json";
        $files = glob($pattern);
    ...
    private function getTodaysBackups() {
        $date = date('Y-m-d');
        $pattern = $this->backupDir . "/gm-monsters_daily_{$date}.json";
        $files = glob($pattern);
    ...
    public function getStats() {
        try {
            ...
            $backups = glob($this->backupDir . '/gm-monsters_*.json');
            if (!$backups) {
                return $stats;
            }
     ...
     */
    public function getBackups($fileName = 'gm-monsters.json') {
...
function createMonsterBackup($filePath, $type = 'auto') {
    $backupHelper = new MonsterBackupHelper();
