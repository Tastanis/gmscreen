...
| `startTurn` | Full | Requests that the resolved target (usually `target:"self"`) start a combat turn. Fields: `target` (`"self"` default), `condition` (`"enemyPickNoActive"` default for Hesitation-style turn stealing, `"pickPhase"`, or `"any"`), `confirmOnInvalid` (default true), optional `invalidMessage`. The runner preflights this before action cost spend so invalid use can warn before the heroic resource is spent. |
...
| `startTurn(payload)` | `{ placementId, condition, confirmOnInvalid, invalidMessage, preflight? }` | `{ started, valid, reason, canceled?, accepted? }`; preflight validates/confirms before resource spend, non-preflight starts the requested combatant's turn. Confirmed allied overrides may replace an active turn or restart a completed combatant; enemy turns remain GM-only. |
