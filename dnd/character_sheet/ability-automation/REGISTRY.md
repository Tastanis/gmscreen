...
| `startTurn(payload)` | `{ placementId, condition, confirmOnInvalid, invalidMessage, preflight? }` | `{ started, valid, reason, canceled?, accepted? }`; preflight validates/confirms before resource spend, non-preflight starts the requested combatant's turn. |
