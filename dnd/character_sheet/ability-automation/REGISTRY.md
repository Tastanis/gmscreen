...
|---|---|---|
| `usageLimit` | Full | Optional top-level object checked before resource spend or `startTurn` effects. Supports `scope: "round" | "turn" | "encounter"`, `key`, `target: "self"`, and optional `message`. Use `{ "scope": "round", "key": "hesitation-is-weakness", "target": "self" }` for once-per-round abilities. |
...
|---|---|---|
| Malice auto-spend | `category` is `villain_action`, `malice`, or a Malice-costed `triggered_action` | Reads cost from `ability.resource_cost` (first integer), calls `window.MaliceTracker.spend(cost)`. Confirm dialog if pool is short. Triggered actions with no numeric Malice cost spend nothing. |
...
| `teleport` | Full | Reuses the slide-shaped destination picker; the final computed range is displayed prominently and the teleport-colored legal-cell highlight covers any cell within Chebyshev distance. Supports embedded `spend` to increase distance before the picker opens. Use `target: "self"` for caster teleports; an unscoped teleport on a sheet action targeted at `Self` safely resolves to the caster instead of a triggering attacker. No stability or size penalty. Clicking an occupied cell still routes through the slide-style collision path — pick an empty cell to follow the rules. |
| `swap` | Full | One confirmed V2 batch exchanges caster/target columns, rows and floors. Failure rejects before success/zone callbacks. Non-equal sizes remain allowed (GM corrects manually). Forced movement; no walking triggers. |
| `abilityTest` | Full | Opens the standard 2d10 roll modal for a non-damaging test. Supports `label`, `attribute`, `skill`, `bonus`, `rollFormula`, and `text`. Matching trained skills add +2 plus any extra sheet skill bonus; if `skill` is omitted, the runner tries to match the skill from `label`. The table interprets success; pair with `ifPrompt` for follow-up automation. |
| `freeStrike` | Full | "By" entity defaults to the most recent target group's first member (the creature being told to free-strike). PCs roll `2d10 + max(M, A)`. Monsters use their stat-block `free_strike` value when present, otherwise fall back to M/A attributes. Prompts for the "against" target via the standard picker, with the by-entity as source, shows a draggable VTT target prompt, and applies damage through the normal automation damage path. Set `asPowerRoll: true` for PC self-strikes that should use the standard power-roll modal and roll suggestions instead of the board-side quick roll. |
...
| `resourceGain` | Full | Mutates the caster's own `hero.resource.value` on their sheet. If the JSON names a specific resource and the caster's resource bar has a different title, falls back to a chat reminder. Floors at 0. **Monster behavior:** the monster context omits `applyResourceGain`, so this posts a manual chat reminder — monsters have no heroic resource bar. |
| `surgeGain` | Full | Mutates `hero.surges` on each PC target's character sheet via VTT surge sync. Surges are separate from heroic resources. Floors at 0. **Monster behavior:** the monster context does NOT currently pass `applySurgeGain`, so a monster `surgeGain` posts a manual chat reminder instead of adjusting the target's surges. (Unlike `recoveries`, which is wired; this is a known passthrough gap.) |
...
| `ifDistance` | Full | Branches on the maximum of occupied-square horizontal distance and floor-height difference via the `getDistanceBetween` host callback. This is range, not line of effect. `from`/`to` endpoints: `self`, `source`/`eventSource`, `eventTarget`, `target`, or a named group. `min`/`max` form the square band (`max:1` = adjacent). Evaluates false (runs `else`) when either token has no placement or no board callback is present. |
...
| `floatingText` | Full | Shows the VTT's giant centered floating text banner. Fields: `text` (required), `audience` (`"all"` default, or `"self"` for local browser only), `tone` (`"danger"`, `"gold"`, `"ally"`, `"neutral"`), optional `durationMs`. `"all"` syncs through combat state so other connected clients display the same banner. |
| `startTurn` | Full | Requests that the resolved target (usually `target:"self"`) start a combat turn. Fields: `target` (`"self"` default), `condition` (`"enemyPickNoActive"` default for Hesitation-style turn stealing, `"pickPhase"`, or `"any"`), `confirmOnInvalid` (default true), optional `invalidMessage`. The runner preflights this before action cost spend. `enemyPickNoActive` accepts either team's open pick phase and blocks without spending while another combatant has an active turn. |
| `aura` | Full | Switches a token aura on (`enabled:true`, default) or off (`enabled:false`) on the caster's token (`target:"self"`, default) or the resolved targets (`target:"target"`). `radius` is clamped 1-20 squares (Chebyshev/square). Optional `color` (hex). Auras can carry `affects`, `triggers`, `effects`, and `expires`; the VTT stores those rules on the token aura, moves the aura with the token, and checks live occupants when timing/occupancy triggers (`turnStart`, `turnEnd`, `roundStart`, `roundEnd`, `combatStart`, `combatEnd`, `enter`, `occupantTurnStart`) or event-bus triggers such as `actionUsed`, `damage`, and `powerRoll` fire. Multiple automated auras can coexist on one token and are toggled separately from token right-click settings. Supported aura tick effects: `damage`, `heal`, `temporaryStamina`, `surgeGain`, `condition`, `ifMark`, `floatingText`, `note`, and `other`. Falls back to a chat reminder if the runtime has no token (e.g. Hero Sheet preview). |
...
| `requestTest(payload)` | normalized `requestedTest` fields, selected target snapshots, serialized tier continuation, and tracked resource reservation | Pauses until `{ canceled }` or `{ results: [{ requestId, tier, total, targetIds }], requestIds }`. The VTT owns routing, queueing, reload recovery, and atomic result claiming. |
| `completeRequestedTests(requestIds)` | canonical request IDs whose tier effects finished | Removes completed canonical requests. |
| `refundAbility(payload)` | tracked resource spend, triggered-action spend, action, and source placement | Restores exact tracked character resource or monster malice plus triggered-action availability after whole-batch cancellation. |
| `clearScopedFlag(payload)` | `{ scope, key, sourceId, targetId }` | Removes the matching usage marker when a requested-test batch cancels before effects resolve. |
...
| `getSurges(payload)` | `{ abilityName, actionId }` | `{ current }`; refreshes the caster's live sheet surge count immediately before a power-roll modal opens. |
...
| `spendRecoveryForTarget(payload)` | `{ placementId, recoveries, abilityName }` | `{ spent, recoveryValue, currentRecoveries, name }` or `{ skipped, reason }`; decrements the requested PC sheet before the heal is applied. `heal.recoverySource: "self"` sends the caster placement id here while applying the heal to the selected target. |
...
| `setAura(payload)` | `{ placementId, enabled, radius, color, automation? }` | `{ applied, enabled, radius }` or rejects when the token is off-scene. Triggerable automation auras are stored as separate per-ability aura records. |
| `showFloatingText(payload)` | `{ text, audience, tone, durationMs, sourceId, abilityName }` | `{ shown, audience, text }`; `audience:"all"` syncs a combat turn effect, `audience:"self"` stays local. |
| `startTurn(payload)` | `{ placementId, condition, confirmOnInvalid, invalidMessage, preflight?, abilityName }` | `{ started, valid, reason, canceled?, accepted? }`; preflight validates before resource spend, non-preflight starts the requested combatant's turn. `enemyPickNoActive` permits an allied start during either open pick phase but returns `accepted:false` while a turn is active. A successful Hesitation Is Weakness start broadcasts its special banner through shared combat effects. Enemy turns remain GM-only. |
...
|---|---|---|
| `move` | `{ placementId, sourceId, from: {column,row,width,height,levelId}, to: {...}, distance, movedDistance, kind: "normal", sceneId, perWatcher }` | Fires once per `vtt:token-moved` (normal movement only). `distance` / `movedDistance` is maximum-axis endpoint displacement including floor height (null for an unknown floor). `perWatcher` is a `Map<watcherId, { leaves, enters }>` so a predicate's adjacency filter can resolve relative to its own watcher. |
