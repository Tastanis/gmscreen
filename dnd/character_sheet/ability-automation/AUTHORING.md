...
| Field | Values |
|---|---|
| `label` | modal/chat label for the test |
| `attribute` | `"Might"`, `"Agility"`, `"Reason"`, `"Intuition"`, `"Presence"`, `"Strongest"`, or an attribute array |
| `bonus` | optional flat bonus |
| `rollFormula` | optional dice formula, defaults to `"2d10"` |
| `text` | optional reminder text shown in the roll modal |
...
- **Heroic resource is PC-only; recoveries and surges target the other creature's sheet.** Monsters have no heroic resource: a `spend` card currently falls through to a native `confirm()` dialog (the monster context omits `spendHeroicResource`), and `resourceGain` posts a manual chat reminder. But `recoveries`-based heals act on the **target's** sheet via `spendRecoveryForTarget` (wired for monsters), so a monster ability that heals or drains a PC target's recoveries works. `surgeGain` is meant to work the same way but `applySurgeGain` is not yet passed to monsters, so it currently posts a manual chat reminder.
