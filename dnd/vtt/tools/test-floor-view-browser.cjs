...
(async () => {
  ...
  try {
    ...
    const roster = await gm.evaluate(() => window.vttConfig.playerRoster);
    const shown = (await snapshot()).state.sceneConfig[manifest.test_scene_id].userLevelState;
    for (const id of roster) assert.equal(shown[id]?.levelId, 'test-upper', 'Show players includes every configured offline profile.');
    if (process.env.VTT_TEST_ROSTER) assert.deepEqual(roster, JSON.parse(process.env.VTT_TEST_ROSTER));
    await pc.reload();
