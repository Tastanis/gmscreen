...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => requests[0].respond({ success: true, content_revision: 'initial', data: { cal: { items: [{ id: 'item', name: 'Initial', effectSections: [{ id: 'effect', title: 'Power', text: 'Original', table: { headers: ['Level', 'Result'], rows: [['1', 'First'], ['2', 'Second']], selectedRow: 1 } }] }] } } }));
