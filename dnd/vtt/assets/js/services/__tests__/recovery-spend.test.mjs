...
test('recovery spending sends only the cost and requires a confirmed result',async()=>{
 let body;
 const result=await spendCharacterRecoveries('/sheet','cal',1,{fetchImpl:async(url,options)=>{body=options.body;return {ok:true,json:async()=>({success:true,operationId:body.get('operationId'),spent:0,currentRecoveries:0})};}});
