...
test('normal hooks preserve floors and exclude undo, forced movement, patches, and recovery', () => {
  ...
  const context = { source: 'acknowledgement', event: { type: 'placement.batchApplied', operationId:'accepted-walk',revision:12,payload: { movementKind: 'walk' } } };
  ...
  assert.equal(detail.movementOperationId,'accepted-walk');assert.equal(detail.movementRevision,12);
