...
export {shouldRevealPlacementHitPointValues} from './token-hit-points.js';
...
export {shouldRenderWallDiagonalConnector, getWallDiagonalConnectorSides} from './wall-diagonal.js';
...
export function mountBoardInteractions(store, routes = {}) {
  ...
  // Feature modules describe entity changes with small typed operations.
  // The persistence choke point translates them to canonical Sync V2 commands.
  ...
  if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
    ...
    window.addEventListener('pageshow', (event) => {
      if (event?.persisted) {
        requestCanonicalRecovery('bfcache-restore');
      }
  ...
  function requestCanonicalRecovery(reason = 'manual') {
    recordSyncDiagnostic('recoveryRequests', {
      reason: typeof reason === 'string' ? reason : 'manual',
      currentRevision: tokenMovementRuntime.getRevision?.() ?? 0,
    });
    const recovery = tokenMovementRuntime.recover?.();
    if (recovery && typeof recovery.catch === 'function') {
      recovery.catch((error) => reportSyncFailure(error, 'recovery'));
    }
  ...
  function recoverStaleRenderedToken(placementId) {
    reconcileStaleTokenPointerTarget({
      ...
      requestResync: requestCanonicalRecovery,
    });
  }
  ...
  const handleTokenDrop = (event) => {
    ...
    // Wait for placement.add before sending the character-sheet stamina
    // patch, so it targets an existing canonical entity revision.
  ...
  boardApi.commitDrawingChanges = async (sceneId, edits) => {
    ...
    try {
      const result = await tokenMovementRuntime.submitBoardDomainCommands(drawingCommands(sceneId, edits));
      updateStatus('Drawing changes saved.');
      return result;
    } catch (error) {
      updateStatus(`Drawing changes were not fully saved: ${error?.response?.error || error?.message || 'Connection failed'}. Accepted changes remain visible.`);
      throw error;
    }
  };
  ...
  function syncDrawingsFromState(boardState, sceneId) {
    setDrawingContext(boardApi.getDrawingContext());
    const drawings = boardState?.drawings?.[sceneId || DEFAULT_SCENE_ID] ?? [];
    setDrawingToolDrawings(Array.isArray(drawings) ? drawings : []);
  }
  ...
  async function fetchAndApplyCharacterStamina(placementId, sceneId, pendingSavePromise = null) {
    ...
    try {
      ...
      // A newly dropped token must exist canonically before its stamina
...
function createTemplateTool() {
  const {sanitizeColorValue, toRoundedNumber, normalizeSerializedTemplate, getGridBounds, clampPointToGridBounds, sanitizeWallSquares, clampWallSquares, getMapGridBounds, snapToStep, snapToHalf, snapPointToGrid, normalizeAngle, toRadians, rectangleCenterFromStart, rectangleStartFromCenter, resolveRectangleStart, resolveCircleCenter, clampRectanglePosition, clampRectangleCenter, clampCircleCenter, geometryForTemplate} = createTemplateGeometry(() => viewState);
  ...
  let colorIndex = 0;
  const colorPalette = TEMPLATE_COLORS;
