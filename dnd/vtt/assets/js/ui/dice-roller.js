class VttDiceRoller {
  constructor(triggerId) {
    this.triggerButton = document.getElementById(triggerId);
  ...
  updateQueueDisplay() {
    const displayText =
      this.currentRollQueue.length === 0 ? '(nothing queued)' : this.currentRollQueue.join(' ');
  ...
  calculateRoll() {
    ...
    try {
      ...
      this.publishRollToChat(rollResult);
      this.currentRollQueue = [];
      this.updateQueueDisplay();
    } catch (error) {
  ...
  updateResultDisplay(rollResult) {
    ...
    const resultLabel = hasAdvantage ? 'Result (Advantage)' : 'Result';
    ...
    if (hasAdvantage) {
      const attempts = Array.isArray(rollResult.advantage.attempts)
        ...
        : [];
      const keptIndex =
        typeof rollResult.advantage.keptIndex === 'number' ? rollResult.advantage.keptIndex : 0;
      ...
      if (attempts.length > 0) {
        ...
        const keptTotal =
          keptAttempt && typeof keptAttempt.total === 'number' ? keptAttempt.total : rollResult.total;
  ...
  publishRollToChat(rollResult) {
    if (
      !rollResult ||
      !window.dashboardChat ||
      typeof window.dashboardChat.sendMessage !== 'function'
    ) {
      return;
    }

    const expression = rollResult.expression ||
      (Array.isArray(rollResult.components) ? rollResult.components.join(' ') : '');
    const descriptor = rollResult.advantage && rollResult.advantage.active ? ' (Advantage)' : '';
    const fallbackMessage = `Dice roll${descriptor} (${expression}): ${rollResult.total}`;
...
let diceRollerInstance = null;

export function mountDiceRoller() {
  const triggerId = 'vtt-dice-roller-btn';
  const trigger = document.getElementById(triggerId);
  if (!trigger) {
    return null;
  }

  if (diceRollerInstance && diceRollerInstance.isReady) {
    return diceRollerInstance;
  }

  diceRollerInstance = new VttDiceRoller(triggerId);
  if (!diceRollerInstance.isReady) {
    diceRollerInstance = null;
    return null;
  }

  try {
    window.vttDiceRoller = diceRollerInstance;
  } catch (error) {
