...
if ($_POST) {
    ...
    try {
        // For students, check for level 2 users
        $stmt = $pdo->prepare("SELECT * FROM users WHERE first_name = ? AND last_name = ? AND (level = 2 OR is_teacher = TRUE)");
        ...
        if ($user) {
            ...
            $_SESSION['user_level'] = $user['level'] ?? 2;
            ...
            $_SESSION['message'] = 'No ASL 2 account found with that name. Please create an account first.';
            $_SESSION['message_type'] = 'error';
            header('Location: index.php');
            exit;
        }
