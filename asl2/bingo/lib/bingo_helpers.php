 ...
 *    claim-status.php)
 *  - Shared file storage (asl2/data/bingo_wordlists.json) that captures
 ...
 *    structure returned by asl2_bingo_add_custom_list().
...
if (!function_exists('asl2_bingo_data_dir')) {
    function asl2_bingo_data_dir(): string
    {
        $dir = __DIR__ . '/../../data';
        if (!is_dir($dir)) {
            mkdir($dir, 0775, true);
...
if (!function_exists('asl2_bingo_wordlist_store')) {
    function asl2_bingo_wordlist_store(): string
    {
        $path = asl2_bingo_data_dir() . '/bingo_wordlists.json';
        if (!file_exists($path)) {
...
if (!function_exists('asl2_bingo_load_custom_lists')) {
    function asl2_bingo_load_custom_lists(): array
    {
        $raw = @file_get_contents(asl2_bingo_wordlist_store());
        $decoded = json_decode($raw ?: '[]', true);
        return is_array($decoded) ? $decoded : [];
    }
}

if (!function_exists('asl2_bingo_save_custom_lists')) {
    function asl2_bingo_save_custom_lists(array $lists): void
    {
        file_put_contents(
            asl2_bingo_wordlist_store(),
...
if (!function_exists('asl2_bingo_filter_words')) {
    function asl2_bingo_filter_words($words): array
    {
        if (is_string($words)) {
            $words = preg_split('/[\r\n,]+/', $words);
        }
...
if (!function_exists('asl2_bingo_add_custom_list')) {
    function asl2_bingo_add_custom_list(int $teacherId, string $name, $words): array
    {
        $lists = asl2_bingo_load_custom_lists();
        $filtered = asl2_bingo_filter_words($words);
        $entry = [
        ...
        $lists[] = $entry;
        asl2_bingo_save_custom_lists($lists);
        return $entry;
    }
}

if (!function_exists('asl2_bingo_find_custom_list')) {
    function asl2_bingo_find_custom_list(string $id, int $teacherId): ?array
    {
        foreach (asl2_bingo_load_custom_lists() as $entry) {
...
if (!function_exists('asl2_bingo_fetch_wordlists')) {
    function asl2_bingo_fetch_wordlists(PDO $pdo, int $teacherId): array
    {
        $scrollerLists = [];
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $words = asl2_bingo_filter_words($wordCandidates);
            $scrollerLists[] = [
                'id' => 'scroller:' . $row['id'],
                'sourceId' => (int) $row['id'],
                'name' => $row['name'],
        ...
        foreach (asl2_bingo_load_custom_lists() as $entry) {
            ...
            $customLists[] = [
                ...
                'words' => asl2_bingo_filter_words($entry['words'] ?? []),
...
if (!function_exists('asl2_bingo_merge_word_sources')) {
    function asl2_bingo_merge_word_sources(PDO $pdo, int $teacherId, array $listRefs): array
    {
        $scrollerIds = [];
        $customIds = [];
        ...
        if ($customIds) {
            foreach ($customIds as $customId) {
                $entry = asl2_bingo_find_custom_list($customId, $teacherId);
                if (!$entry) {
                    continue;
                }
                $filtered = asl2_bingo_filter_words($entry['words'] ?? []);
                $wordPool = array_merge($wordPool, $filtered);
                $activeLists[] = [
                    'id' => $entry['id'],
                    'name' => $entry['name'] ?? 'Custom List',
...
if (!function_exists('asl2_bingo_close_active_sessions')) {
    function asl2_bingo_close_active_sessions(PDO $pdo, int $teacherId, int $aslLevel): void
    {
...
if (!function_exists('asl2_bingo_create_session')) {
    function asl2_bingo_create_session(PDO $pdo, int $teacherId, int $aslLevel, array $activeLists, array $wordPool): array
    {
        asl2_bingo_close_active_sessions($pdo, $teacherId, $aslLevel);
        ...
        return asl2_bingo_get_teacher_session($pdo, $teacherId, $aslLevel, true, $sessionId);
    }
}

if (!function_exists('asl2_bingo_decode_json')) {
    function asl2_bingo_decode_json($value, $default = [])
    {
        if ($value === null || $value === '') {
            return $default;
        }
...
if (!function_exists('asl2_bingo_get_teacher_session')) {
    function asl2_bingo_get_teacher_session(PDO $pdo, int $teacherId, int $aslLevel, bool $allowClosed = false, ?int $specificId = null): ?array
    {
...
if (!function_exists('asl2_bingo_get_active_session_by_level')) {
    function asl2_bingo_get_active_session_by_level(PDO $pdo, int $aslLevel): ?array
    {
...
if (!function_exists('asl2_bingo_fetch_called_words')) {
    function asl2_bingo_fetch_called_words(PDO $pdo, int $sessionId): array
    {
...
if (!function_exists('asl2_bingo_deterministic_card')) {
    function asl2_bingo_deterministic_card(array $wordPool, int $sessionId, int $userId): array
    {
...
if (!function_exists('asl2_bingo_get_or_create_card')) {
    function asl2_bingo_get_or_create_card(PDO $pdo, array $session, int $userId): array
    {
        ...
        if ($card) {
            $cardWords = asl2_bingo_decode_json($card['card_words'], []);
            if (count($cardWords) !== 25) {
                $cardWords = asl2_bingo_deterministic_card($wordPool, $sessionId, $userId);
            }
            $marks = asl2_bingo_decode_json($card['marks'], []);
            return [
                'id' => (int) $card['id'],
...
if (!function_exists('asl2_bingo_save_card_marks')) {
    function asl2_bingo_save_card_marks(PDO $pdo, int $cardId, array $marks): void
    {
...
if (!function_exists('asl2_bingo_sanitize_marks')) {
    function asl2_bingo_sanitize_marks($marks): array
    {
        if (!is_array($marks)) {
            return [];
        }
...
if (!function_exists('asl2_bingo_has_pattern')) {
    function asl2_bingo_has_pattern(array $marks): bool
    {
        if (count($marks) < 5) {
            return false;
        }
...
if (!function_exists('asl2_bingo_evaluate_claim')) {
    function asl2_bingo_evaluate_claim(array $cardWords, array $marks, array $calledWords): array
    {
        $calledSet = [];
...
if (!function_exists('asl2_bingo_record_claim')) {
    function asl2_bingo_record_claim(PDO $pdo, array $session, int $userId, array $card, array $marks, array $calledWords, string $studentName): array
    {
        $sessionId = (int) $session['id'];
        $evaluation = asl2_bingo_evaluate_claim($card, $marks, $calledWords);
...
if (!function_exists('asl2_bingo_format_session_payload')) {
    function asl2_bingo_format_session_payload(array $session, array $calledWords, array $claims = []): array
    {
        $wordPool = asl2_bingo_decode_json($session['word_pool'], []);
        ...
        return [
            ...
            'activeLists' => asl2_bingo_decode_json($session['selected_word_source'], []),
