...
Updated September 9, 2026, against the 1.19.137 working implementation.
...
| Inventory save reliability | Atomic file replacement, precise refresh revision, dirty-draft retention and per-field save ordering implemented. | Updated-client field edits now reject stale revisions (two-browser test passes). Delete/move guards now pass two-editor checks; whole-item/image guards, moves with unsaved edits and recovery of unconfirmed writes remain; no separate backup system. |
...
The latest complete suite run has 795 passing tests across 105 files (1.19.137).
