...
// Get all students and their progress (ASL 1 students only)
try {
    $stmt = $pdo->prepare("
        SELECT 
            u.id,
        ...
        WHERE u.is_teacher = FALSE AND (u.level = 1 OR u.level IS NULL)
...
<body>
    <div class="container">
        <header>
            <h1>ASL 1 Hub - Teacher Dashboard</h1>
            <div class="user-info">
                <span>Welcome, <?php echo htmlspecialchars($_SESSION['user_first_name'] . ' ' . $_SESSION['user_last_name']); ?>!</span>
                <a href="../asl2/teacher_dashboard.php" class="form-button" style="margin-right: 10px; background: linear-gradient(135deg, #805ad5 0%, #6b46c1 100%);">Switch to ASL 2</a>
